                ...
                <div class="text-center mt-6 relative">
                    <ButtonComponent  @click.prevent="submitForm()" type="submit">
                        <div class="flex align-center justify-center align-middle space-x-343,143,0.8)] w-full h-full relative top-0 left-0 right-0">
                            ...
                            </svg> {{  ( this.processingForm  ) ? 'Processing' : 'Login'  }}
                        </div>
                    </ButtonComponent>
                </div>
            </template>
            ...
            <template v-slot:auth-navigations>
                <div class="flex flex-wrap mt-6 relative">
                    ...
                    <div class="w-1/2 text-right">
                        <router-link to="/register" class="text-slate-200 text-sm">Create An Account</router-link>
                    </div>
                </div>
            </template>
        </FormComponent>
...
export default {
    ...
    methods: {
        ...
        /**
         * Submit Login Request and process all form data
         */
        async submitForm() {
            ...
            if ( this.form.password !== "" && this.form.email !== "" ) {
                ...
                try {
                    let __response = await axios.post('/auth/login', this.form)
